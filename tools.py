...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    for h in filtered:
        line = (
            ...
            f"{h['rating']}⭐ | {format_price(h['price_per_night'])}/đêm"
        )
        result.append(line)
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for k, v in parsed.items():
            lines.append(f"- {k}: {format_price(v)}")

        lines.append("---")
        lines.append(f"Tổng chi: {format_price(total)}")
        lines.append(f"Ngân sách: {format_price(total_budget)}")

        if remaining >= 0:
            lines.append(f"Còn lại: {format_price(remaining)}")
        else:
            lines.append(f"Vượt ngân sách {-remaining:,}đ! Cần điều chỉnh.".replace(",", "."))

        return "\n".join(lines)

    except Exception:
        return "Lỗi format expenses. Đúng dạng: 'vé máy bay:890000, khách sạn:650000'"
